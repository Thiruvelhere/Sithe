...
import { setProof } from '../../core/sdkContext'
    ...
    exec(`snarkjs wtns calculate ${wasmPath} ${inputPath} ${witnessPath}`, (err1, stdout1, stderr1) => {
      ...
      exec(`snarkjs groth16 prove ${zkeyPath} ${witnessPath} ${proofPath} ${publicPath}`, async (err2, stdout2, stderr2) => {
        ...
        try {
          ...
          const zkProof = '0xzk_' + proofHash.slice(0, 24)

          setProof(zkProof)
          console.log('[ZYPHER]: 🏷️ ZK Stamp:', zkProof)
          resolve(zkProof)
        } catch (err3) {
          console.error('[ZYPHER]: ❌ Failed to read proof.json', err3)
